...
// 国际化
block_basekit_server_api_1.basekit.setI18n({
    defaultLocale: 'zh-CN',
    messages: {
        'zh-CN': {
 ...
 * 分页拉取所有任务
 */
async function fetchAllTasks(baseUrl, token, fetchFn) {
    const allTasks = [];
    let pageToken = '';
    do {
...
block_basekit_server_api_1.basekit.addAction({
    ...
    execute: async function (args, context) {
        ...
        const matchedTask = tasks.find((t) => t.summary === taskName);
        if (!matchedTask?.guid) {
            return {
                success: false,
        ...
        for (const dept of departments) {
            ...
            try {
                const addResp = await fetch(`${baseUrl}/open-apis/task/v2/tasks/${taskGuid}/add_tasklist?user_id_type=open_id`, {
                    method: 'POST',
                    headers: {
    ...
    resultType: {
        ...
        properties: {
            success: { type: block_basekit_server_api_1.ParamType.Boolean, label: '是否成功' },
            message: { type: block_basekit_server_api_1.ParamType.String, label: '结果信息' },
            details: { type: block_basekit_server_api_1.ParamType.String, label: '详细情况' },
        },
    },
});
